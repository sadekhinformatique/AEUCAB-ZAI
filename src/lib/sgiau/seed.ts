...
const DEPARTMENTS: Record<string, string[]> = {
  ...
  Droit: ["Droit public", "Droit privé", "Sciences criminelles"],
  Économie: ["Économie", "Gestion", "Commerce"],
  Lettres: ["Philosophie", "Histoire", "Langues"],
  Médecine: ["Médecine générale", "Pharmacie"],
  Ingénierie: ["Génie civil", "Génie électrique", "Génie logiciel"],
}
...
export async function seedDatabase(force = false) {
  ...
  for (let i = 1; i <= TOTAL_MEMBERS; i++) {
    ...
    const department = pick(DEPARTMENTS[faculty])
  ...
  for (const m of members.filter((x) => x.status === "ACTIVE")) {
    ...
    const amount = cotAnnual.defaultAmount + (m.level.startsWith("M") ? 1000 : 0)
